...
func Test_EncodeOptionCompact(t *testing.T) {
	var examples = []struct {
		label  string
		input  Option[Compact[Numeric]]
		expect []byte
	}{
		{label: "Encode Option(true, Compact(MaxUint64)", input: NewOption[Compact[Numeric]](ToCompact(uint64(math.MaxUint64))), expect: []byte{0x1, 0x13, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff}},
	}
...
func Test_DecodeOptionCompact(t *testing.T) {
	var examples = []struct {
		...
		input         []byte
		bufferLenLeft int
		expect        Option[Compact[Numeric]]
	}{
		{
			label:         "Decode Compact(maxUint64)",
			input:         []byte{0x1, 0x13, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},
			expect:        NewOption[Compact[Numeric]](ToCompact(NewU128(uint64(math.MaxUint64)))),
			bufferLenLeft: 0,
		},
	}
	...
	for _, e := range examples {
		t.Run(e.label, func(t *testing.T) {
			...
			result, err := DecodeOption[Compact[Numeric]](buffer)
